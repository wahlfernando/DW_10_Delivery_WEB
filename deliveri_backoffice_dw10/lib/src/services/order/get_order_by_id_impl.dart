...
class GetOrderByIdImpl implements GetOrderById {
  ...
  Future<OrderDto> _orderDtoParse(OrderModel order) async {
    ...
    return OrderDto(
      ...
      date: order.date,
      status: order.status,
      orderProducts: resposnses[2] as List<OrderProductDto>,
      user: resposnses[1] as UserModel,
      address: order.address,
      cpf: order.cpf,
      paymentTypeModel: resposnses[0] as PaymentTypeModel,
    );
  }
  ...
  Future<List<OrderProductDto>> _orderProductParse(OrderModel order) async {
    ...
    final productsFuture = order.orderProducts
        .map((e) => _productRepository.getProduct(e.productIid))
        .toList();
